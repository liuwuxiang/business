...
mui.plusReady(function() {
	...
	function getCommodityGuiGe(type) {
		...
		$.ajax({
			url: mMain.url + "/wnk_business_app/v1.0.0/getCommodityGuiGe",
			type: "POST",
			dataType: 'json',
			data: {
				...
				if(data.status == 0) {
					...
					for(var index = 0; index < list.length; index++) {
						...
						var html = "<li>" +
							...
							"<input type=\"button\" value=\"修改\" class=\"setAction\" name=\"" + obj.id + "|" + obj.specifications_name + "\"/>" +
	...
	function addGuiGeNetwork(guige_name) {
		...
		jQuery.support.cors = true;
		$.ajax({
			url: mMain.url + "/wnk_business_app/v1.0.0/addCommodityGuiGe",
			type: "POST",
			dataType: 'json',
			data: {
				"business_id": storage["business_id"],
	...
	function setGuiGeName(guige_name, guige_id) {
		...
		jQuery.support.cors = true;
		$.ajax({
			url: mMain.url + "/wnk_business_app/v1.0.0/updateGuiGeName",
			type: "POST",
			dataType: 'json',
			data: {
				"business_id": storage["business_id"],
	...
	function startOrEndGuiGeNetwork(guige_id, type) {
		...
		jQuery.support.cors = true;
		$.ajax({
			url: mMain.url + "/wnk_business_app/v1.0.0/updateGuiGeState",
			type: "POST",
			dataType: 'json',
			data: {
				"business_id": storage["business_id"],
